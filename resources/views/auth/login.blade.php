<x-main>
    ...
    <div class="text-white font-semibold flex justify-between items-center pl-20 pr-60 mt-20 w-full absolute z-50">
        <div class="w-[55%] ">
            <form action="{{ route('login') }}" method="post" class="flex flex-col items-center w-full border-4 rounded-xl px-5 py-8 border-[#40CE9B] ">
                @csrf
                <div class="flex flex-col items-center w-full">
                    <label for="">Nama Pengguna</label>
                    <input type="text" name="email" class="border-b-[7px] border-spacing-4 border-[#40CE9B] mt-3 w-full py-2 text-black bg-[#DDE9E4] rounded-full" value="{{ old('name') }}">
                    @error("email")
                ...
                <div class="flex flex-col items-center my-5 w-full">
                    ...
                    <input type="text" name="password" class="border-b-[7px] border-spacing-4 border-[#40CE9B] mt-3 w-full py-2 text-black bg-[#DDE9E4] rounded-full" value="{{ old('password') }}">
                    @error("password")
                     <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
